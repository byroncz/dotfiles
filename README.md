...
DEVKIT-80), sin lanzar ningún agente: una sola consulta a Notion por
refresco, `notion.sh activas <código>`. Pensado para no dedicar una pantalla
al tablero de Notion. `--seguir` la refresca cada `DEVKIT_TABLERO_INTERVALO`
(30 s, no los 3 s de `--estado --seguir`: cada vuelta paga una consulta real,
sin la caché de 30 s que sí protege a "bloquea a"/Épica en `--estado`).
