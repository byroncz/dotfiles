...
| `/opt/devkit/scripts/pr-guard.sh` | Hook `PreToolUse` de `settings.json`: revisa el comando `Bash` completo, en cualquier posición de sus argumentos, y bloquea (salida 2) aprobar un PR, mergearlo sin `--auto` o empujar a `main`, aunque el comando evada el deny por prefijo (`git -C <dir> push`, `gh api` crudo). `--test` corre su autoprueba. |
...
bloquea `gh pr review --approve`, `gh pr merge` sin `--auto` y `git push` a
`main` sin importar cómo escriba el comando, y las reglas `deny` de
`settings.json` quedan como segunda barrera. La compuerta real es GitHub (sin
autoaprobación, ruleset de `main`); el detalle está en `docs/ARCHITECTURE.md`,
secciones 8.2 y 12b.
