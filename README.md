...
| `/opt/devkit/scripts/prompt-status.sh` | Arma la línea dinámica del prompt (DEVKIT-63; ver "Dentro del contenedor" más abajo para el detalle de cada segmento). Lo corre `starship` en un solo módulo `custom`, no la shell a mano. `--test` corre su autoprueba. |
...
| `/opt/devkit/scripts/devkit-run.sh` (alias `devkit-run`, solo en la shell interactiva) | Único punto de lanzamiento de una skill: `devkit-run [--modelo <alias>] [--esfuerzo <low\|medium\|high\|xhigh\|max>] [--forzar] [--seguir] <skill> <Clave> [texto extra...]` la corre en segundo plano con `nohup setsid` desde `/workspace`, sin que copies el comando largo a mano, y vuelve en cuanto confirma que arrancó. `setsid` (DEVKIT-82) nace en su propia sesión de proceso: sin ella, el worker comparte el grupo de proceso de la terminal -un `&` sin `set -m` no crea uno propio- y una señal real lo alcanzaría igual que a este script. `--seguir` (DEVKIT-82) no vuelve: se queda mostrando `--estado` hasta que ese mismo lanzamiento termine o hasta Ctrl-C, que solo cierra el monitor -el worker, ya aislado con `setsid`, sigue-; la última línea es el resumen de `watch.log` de ese lanzamiento. Si el worker muere sin dejar esa línea (`SIGKILL`, sin memoria) el monitor no espera para siempre: pasado `DEVKIT_MARGEN_LANZAMIENTO_MUERTO` (10 s) desde que `kill -0` del PID del worker empieza a fallar, lo dice y sale con un código distinto de cero (H3 de `pr-review` en DEVKIT-82). Antes de lanzar, si ya hay un worker o un `claude -p` de ese mismo prompt vivo o esperando el candado, no lanza: imprime `ya hay un lanzamiento de "<prompt>" en curso (pid <n>, log <ruta>); síguelo con devkit-run --estado` (el log sale de la última línea `lanzando` de ese prompt en `watch.log`) y sale con 68 (DEVKIT-79: un falso "no arrancó" -ver más abajo- llevó a relanzar la misma card tres veces sin saber que la anterior seguía viva). `--forzar` salta esa comprobación. Antes de lanzar espera el marcador `/run/devkit/ready` que escribe el arranque del contenedor (hasta `DEVKIT_READY_TIMEOUT`, 120 s); si no aparece, no lanza, lo explica y sale con 69. Tras lanzar mira al worker hasta `DEVKIT_ARRANQUE_ESPERA` segundos (5): vivo, busca su `claude -p` entre los descendientes del worker (no el primer proceso del sistema con el mismo prompt: con dos lanzamientos vivos a la vez, ese primero podía ser el de uno anterior, DEVKIT-79) y dice si corre o si el worker espera el candado; muerto sin resumen `terminado`, imprime las últimas líneas del log y las alarmas, deja `ALARMA: no arrancó` en `watch.log` y sale con 70 (DEVKIT-57: un `task-start` lanzado con el editor recién abierto imprimió su PID y nunca corrió). Cada lanzamiento deja antes en `watch.log` la línea `<id> lanzando (origen=<quién>) modelo=<alias> esfuerzo=<x> ronda=<n>: "<prompt>" log=<log>`, donde `<id>` es el nombre del log, el origen sale de `DEVKIT_ORIGEN` (`task-close.sh` pone `task-close`, `watch.sh` pone `bucle`) o, si no viene, del primer `claude -p /<skill>` entre los procesos padre (`epic-plan`), y si no hay ninguno, `humano`, y modelo/esfuerzo/ronda ya están resueltos por rol antes de escribirla (DEVKIT-81: `run_skill` en `watch.sh` mueve su `--rol` a este punto, antes del candado). Una línea vieja sin esos tres campos -un `watch.sh` en memoria, sin `recreate` todavía- se sigue leyendo, con la columna `MODELO` en `-`. `devkit-run --estado` lee esas líneas y muestra una tabla de los últimos 20 lanzamientos (`DEVKIT_ESTADO_FILAS`): skill, card, quién lanzó, hace cuánto, estado, modelo (`sonnet/high r1`, desde que la fila aparece, no solo al terminar) y detalle, sin lanzar ningún agente. El estado es `en curso` (su proceso vive, se lanzó hace menos de `DEVKIT_ESTADO_GRACIA` segundos —120— aunque su `claude -p` aún no exista, o espera el candado; si ya pasó `DEVKIT_WATCH_SKILL_TIMEOUT` sin resumen, el detalle suma `lento`, la misma alarma que deja `watch_long_running` en `watch.log`), `terminó`, `error` (rc distinto de cero, log escrito sin resumen ni proceso, o un `task-start` que "terminó" pero dejó su card `En progreso` sin PR ni bloqueo -corte silencioso de DEVKIT-63, respaldo de DEVKIT-77-), `bloqueada` (`task-block.sh` bloqueó su card después de lanzarlo; el detalle es el motivo), `no arrancó` (sin proceso, log ni resumen pasado el margen) o `sin registro` (DEVKIT-81, regla sin excepción: un `claude -p` vivo en `ps` sin ninguna línea `lanzando` que lo explique, salvo la sonda de modelo -`-p "ok"`- y la lectura de cuota -`-p "/usage"`-, que no son skills y se descartan por su prompt). `devkit-run --estado --seguir` la refresca cada 3 s (`DEVKIT_ESTADO_INTERVALO`) hasta Ctrl-C, sin parpadear (cuadro armado en memoria, cursor de vuelta al origen y borrado solo al final, oculto con `tput civis`/`cnorm`, DEVKIT-81), con una cabecera que suma un girador y `bucle: vivo, último tick hace <N>` o `bucle: SIN SEÑAL` si `watch.sh` no aparece en `ps` o su último tick "consultando GitHub" pasa el doble de su intervalo. Cada vez que se muestra, deja en `/run/devkit/alarmas-vistas` cuántas líneas tenía `watch.log`: el segmento `!<k>` del prompt cuenta solo las `ALARMA:` posteriores a esa marca (DEVKIT-63). Una card en `Lista para merge` suma a su fila "bloquea a: <Claves>" con las cards en `Lista` que la tienen en `Depende de` (`notion.sh bloqueos`, cacheado 30 s, una sola llamada por refresco, nunca en línea). Si entre las filas hay Tareas de más de una Épica `En progreso`, la tabla se agrupa: un encabezado `Épica <Clave>: <Título>` por cada una (en el orden en que aparecen), y un bloque final `(sin Épica)` para las filas sin Padre activo, sin Clave, o cuya card ya está `Hecha` aunque su Épica siga activa; con una sola Épica, o ninguna, la tabla queda plana como antes (`notion.sh epicas`, misma caché de 30 s que `bloqueos`, DEVKIT-80). `devkit-run --agentes` imprime solo el número de filas `en curso`, sin la tabla ni el bloque Consumo, para el segmento `agentes:<a>` del prompt: bajo 20 ms contra los ~90 ms de `--estado` completo (DEVKIT-63). Antes de resolver el modelo, hace `git fetch origin main` (con `timeout 5`; si falla, solo avisa que no pudo comprobar y sigue) y avisa por stderr y deja `ALARMA: workspace <n> commit(s) detrás de origin/main` en `watch.log` si el workspace quedó atrás de `origin/main` (`git rev-list --count main..origin/main`): en modo dev es la señal de que va a lanzar con `roles.toml` o con este mismo script viejos, el caso real que dejó a DEVKIT-66 sin la escalera de modelos el 2026-09-16. El `fetch` es necesario porque, sin él, la comparación usa la última referencia local de `origin/main` -que puede estar vieja si nada más en el contenedor la actualiza- y no detecta un avance reciente del remoto (hallazgo de `pr-review` en el PR #53). Lo lanza con el modelo y el esfuerzo que le tocan por su papel en el flujo (resueltos en `$DEVKIT_ROLES_FILE` si se define, luego `.devkit/roles.toml` del proyecto si existe, luego `devkit/agents/roles.toml` del template, al final `/opt/devkit/template/agents/roles.toml`; ver "Qué declara cada proyecto") y el mismo candado que usa `watch.sh` para no pisarle la rama a otra skill. En `task-fix` y `task-document`, el modelo y el esfuerzo dependen además de la ronda del PR si el rol declara `rondas` (DEVKIT-61, ver "Escalera de modelos por ronda"); la ronda sale también en la línea `lanzado` y en el resumen como `ronda=<n>`. `--modelo`/`--esfuerzo` anulan el rol resuelto (y la ronda) para ese lanzamiento puntual, sin tocar `roles.toml`; la línea de resumen lo marca `(anulación manual)`. Log en `/run/devkit/<skill>-<n>.log`; al terminar, agrega a `watch.log` una línea con modelo, esfuerzo, costo y turnos, más las mismas alarmas de `watch.sh` (error, skill lenta, pregunta abierta) y, si el `result` es una pregunta abierta, bloquea la card con `task-block.sh` y un motivo forzado -salvo que la card ya esté `Hecha` (DEVKIT-76): ahí no bloquea, solo deja `ALARMA: terminó sin estado observable sobre una card ya Hecha; no se bloquea (<motivo>)` en `watch.log`, porque un relanzamiento de más sobre una card cerrada, mergeada y documentada no debe volver a `Bloqueada`; la misma barrera cubre también la falta de acceso a Notion, y siempre deja una sola `ALARMA` por evento, nunca dos-. La barrera de pregunta abierta (`pregunta_abierta`) ya no solo mira si el texto entero termina en "?" (DEVKIT-50): desde DEVKIT-77 también reconoce, en el último párrafo del `result`, una línea que empieza por "¿", "Opciones:" seguida de líneas numeradas, o las frases fijas "¿Cómo quieres que siga"/"¿Qué prefieres" -formas que el cierre real de DEVKIT-63 ("¿Cómo quieres que siga? Opciones: 1. [...] 2. [...] 3. [...] antes de decidir.") usaba sin terminar en "?", así que no bloqueaba la card. `devkit-run --pregunta-abierta "<resultado>"` expone esa misma regla como subcomando puro (sale 0 si es una pregunta abierta, 1 si no) para que `watch.sh` la use también en su camino `--sync` (DEVKIT-77 H1), en vez de duplicarla con un `grep` propio. `devkit-run task-close ...` y `devkit-run task-block ...` no lanzan modelo: corren en primer plano `task-close.sh` y `task-block.sh` (DEVKIT-55); `--sync` hace lo mismo con los prompts `/task-close` y `/task-block`, que todavía pide un `watch.sh` anterior hasta el próximo `devkit recreate`. Exporta `DEVKIT_MODEL` y `DEVKIT_EFFORT` al `claude -p` que lanza, con el modelo y el esfuerzo que recibió de verdad (rol, caída en `frontera`, `--modelo`/`--esfuerzo` o `DEVKIT_MODELO_FORZADO`), no los que traiga el entorno: de ahí sacan las skills las marcas "Implementado/Revisado/Documentado con <modelo>, esfuerzo <x>" (DEVKIT-58, ver "Modelo y esfuerzo visibles"). Exporta también `DEVKIT_SCRIPTS_DIR` (su propio directorio) y `DEVKIT_RUN_DIR` al `claude -p` que lanza (y `DEVKIT_LOCK_HELD=1` en `--worker`, que corre con el candado tomado; `watch.sh` la pasa a `--sync` por lo mismo: así `task-block.sh` guarda el `wip` sin pedir otra vez el candado; `watch.sh` pasa además `DEVKIT_LANZADOR=watch`, que un lanzamiento en segundo plano borra, para que `task-fix` sepa si lo lanzó el bucle), para que una skill que invoca otro script por ruta llegue a la misma copia y no a la de la imagen, vieja en modo dev. Arma el entorno del `claude -p` hijo con `env -i` y una lista blanca, y antes de lanzar prueba con `claude mcp list` que Notion está conectada; si no, no lanza (ver "Entorno del `claude -p` hijo y comprobación de Notion", DEVKIT-65). Si el modelo resuelto sale vacío, no lanza: sale con 65 y deja `ALARMA: modelo vacío` en `watch.log` (antes la CLI moría con un 400 en el primer turno, DEVKIT-55). `task-next.sh` y `epic-plan` lo usan para lanzar la siguiente/primera hija como proceso aparte, así corre con el rol que le toca; `epic-plan` lo llama por ruta explícita (`"${DEVKIT_SCRIPTS_DIR:-/opt/devkit/scripts}/devkit-run.sh"`), no por el alias `devkit-run`, porque su `SKILL.md` corre en el Bash no interactivo de `claude -p`, que no carga `zshrc` (DEVKIT-54). `watch.sh` lo usa también (ver más abajo). `devkit-run --otros-agentes` responde si otro agente ya ocupa el workspace: imprime los procesos `claude -p` ajenos y sale 0 si está libre, 1 si no. Es lo que debe usar una skill en vez de un `pgrep -f <Clave>`, que devuelve como ajenos los cuatro procesos propios del lanzamiento (el `--worker`, su subshell, su vigilante y el `claude -p` de uno mismo) porque la Clave viaja en sus argumentos; ese falso positivo bloqueó una card sin motivo en DEVKIT-54. Si corre dentro de un agente, imprime antes una línea `propio: <pid> claude -p "<prompt>"` con su propio proceso ya identificado (DEVKIT-77): es el mismo proceso que aparece en cualquier `ps` con `PPID` 1 (el `nohup` de quien lo lanzó), no un segundo agente; desconfiar de este resultado y correr un `ps` aparte para "confirmarlo" cortó DEVKIT-63 sin PR y sin bloquear. Todo `ps` que lee argumentos (acá y en `watch.sh`) pide ancho ilimitado con `-ww`: sin ella, una terminal integrada como la del editor exporta `COLUMNS`/`LINES` y `ps` corta cada línea a ese ancho aunque la salida vaya a una tubería, dejando la ruta del log fuera y el proceso invisible para `--estado` (DEVKIT-79). `devkit-run --siguiente-modelo <alias>` imprime el modelo disponible que sigue a `<alias>` en `frontera` (tras el último, el primero); lo usa `watch.sh` para relanzar un `task-fix` vacío, y `--sync` toma ese modelo de `DEVKIT_MODELO_FORZADO`. `devkit-run --tablero [--seguir]` (DEVKIT-82) muestra las cards activas del proyecto (`Lista`, `En progreso`, `Revisión automática`, `Lista para merge`, `Bloqueada`) en una tabla de consola -Clave, Estado, Tipo, PR y "bloquea a" (la misma columna de `--estado`, DEVKIT-63)-, agrupada por Épica de origen cuando hay más de una `En progreso` (mismo criterio que `--estado`, DEVKIT-80), sin lanzar ningún agente. `notion.sh activas <código>` trae la lista en cada vuelta; a diferencia de `--estado`, que solo dispara el refresco de "bloquea a"/Épica en segundo plano porque ya tiene algo que mostrar mientras llega (los procesos, el log), `--tablero` asegura esas dos cachés en primer plano antes de pintar, así que con las cachés frías la primera vuelta paga hasta tres consultas -activas, bloqueos y epicas-, no una sola (H1 de `pr-review` en el PR #59 de DEVKIT-82: antes salía plano y sin "bloquea a" la primera vez). `--seguir` la refresca cada `DEVKIT_TABLERO_INTERVALO` (30 s, no los 3 s de `--estado --seguir`, para no gastar el límite de peticiones de Notion). `--test` corre su autoprueba. |
...
El prompt (DEVKIT-63) es una sola línea, sin red y bajo 50 ms
(`starship timings`, medido en `/workspace`):
`[arrancando ]<proyecto>:<template> <rama> [±n] [↑m] [agentes:a] [!k] >`.
Cada segmento entre corchetes se omite cuando vale 0. De dónde sale cada uno:
...
```sh
devkit-run --estado --seguir   # refresca cada 3 s, sin parpadear
...
`devkit-run --seguir <skill> <Clave> [texto extra...]` (DEVKIT-82) lanza
igual que el uso normal y, en vez de devolver el prompt, se queda mostrando
`--estado` -mismo redibujo sin parpadeo, cada `DEVKIT_ESTADO_INTERVALO`- hasta
que ESE lanzamiento (por su id, el nombre del log) deja su línea de cierre en
`watch.log`, que es la última línea que imprime (`terminado [...]: modelo=...
costo=...`). Ctrl-C no mata el lanzamiento: el worker nace con `setsid`, en su
propia sesión de proceso, desde antes de entrar a este bucle -sin eso,
comparte el grupo de proceso de la terminal con este script, porque un `&` sin
`set -m` no crea uno propio, y una señal real lo alcanzaría igual que al
monitor-, así que Ctrl-C solo cierra el monitor, y lo dice en una línea:

```sh
devkit-run --seguir task-start DEVKIT-82
# lanzado: /task-start DEVKIT-82
# modelo=sonnet esfuerzo=high ronda=1 log=/run/devkit/task-start-3.log pid=1234
# arrancó: claude -p vivo (pid 1240)
# devkit-run --seguir task-start-3  16:20:04 |  (cada 3s; Ctrl-C solo cierra el monitor)
# bucle: vivo, último tick hace 40s
#
# SKILL          CARD        LANZÓ       HACE    ESTADO        MODELO          DETALLE
# task-start     DEVKIT-82   humano      12s     en curso      sonnet/high r1  -
# ^C
# devkit-run: Ctrl-C cierra el monitor; el lanzamiento "task-start-3" sigue en curso (síguelo con devkit-run --estado)
```

`devkit-run --tablero [--seguir]` (DEVKIT-82) muestra las cards activas del
proyecto (`Lista`, `En progreso`, `Revisión automática`, `Lista para merge`,
`Bloqueada`) en una tabla de consola -Clave, Estado, Tipo, PR y "bloquea a"
(la misma columna de `--estado`, DEVKIT-63)-, agrupada por Épica de origen
cuando hay más de una `En progreso` (mismo criterio que `--estado`,
DEVKIT-80), sin lanzar ningún agente. `notion.sh activas <código>` trae la
lista de cards en cada vuelta; "bloquea a" y la Épica de origen reusan las
mismas cachés de 30 s de `--estado` (DEVKIT-63/DEVKIT-80), pero
`--tablero` las asegura en primer plano antes de pintar en vez de solo
disparar su refresco en segundo plano como hace `--estado`: con las cachés
frías, la primera vuelta paga hasta tres consultas -activas, bloqueos y
epicas-, no una sola (H1 de `pr-review` en DEVKIT-82). Pensado para no
dedicar una pantalla al tablero de Notion. `--seguir` la refresca cada
`DEVKIT_TABLERO_INTERVALO` (30 s, no los 3 s de `--estado --seguir`: cada
vuelta paga una consulta real, sin la caché de 30 s que sí protege a
"bloquea a"/Épica en `--estado`).

```sh
devkit-run --tablero
# CLAVE       ESTADO                TIPO      PR                                      BLOQUEA A
# DEVKIT-81   En progreso           feature                                           
# DEVKIT-82   Lista                 feature                                           
```
...
abierta -incluidas las formas de DEVKIT-77 que no terminan en "?": una línea
que empieza por "¿", "Opciones:" con líneas numeradas y sus frases fijas,
probadas también de punta a punta con el texto real del incidente de
DEVKIT-63-, la delegación de `devkit-run task-block` al script, que un modelo
...
lanzamiento no cuentan como agente ajeno, y que la línea `propio: <pid>
claude -p "<prompt>"` identifica al `claude -p` propio entre los ancestros
sin confundirlo con el vigilante ni con un agente ajeno (DEVKIT-77). De
DEVKIT-57 prueba el origen por procesos padre con una tabla fija, el
siguiente modelo de `frontera`, dos arranques fallidos (sin marcador `ready`:
sale con 69 sin lanzar; `claude -p` que muere al instante: sale con 70 e
imprime el log) y `--estado` con un `watch.log`, un `ps` y una hora fijos, un
caso por cada uno de los cinco estados, incluido `en curso` dos segundos
después de "lanzando" sin proceso y, de DEVKIT-77, un `task-start` que
"terminó" con la card `En progreso` sin PR ni bloqueo, que cuenta como
`error`.
...
- `/task-start` lee el Estado de la card antes de tocar el workspace
  (DEVKIT-76): un relanzamiento por error sobre una card que ya no está
  libre (`Hecha`, `Lista para merge`, `Revisión automática` o `Bloqueada`)
  lo dice en ese mismo turno, sin tocar git ni Notion, en vez de actualizar
  el workspace y preguntar qué hacer (un relanzamiento así sobre una card
  `Hecha`, antes de esta card, gastó 8 turnos y terminó en `Bloqueada`
  porque la barrera de pregunta abierta no distinguía una card ya cerrada,
  ni la comprobación del Estado corría si el árbol estaba sucio o había
  otro agente vivo). `En progreso` sin `Rama` se trata como `Lista`;
  `Backlog` responde que el humano debe moverla a `Lista`. Esa misma
  barrera (`forzar_task_block` en `devkit-run.sh`) tampoco bloquea una card
  `Hecha` aunque el `result` parezca una pregunta o falte acceso a Notion:
  deja una sola alarma en `watch.log`, citando el motivo real, y no llama a
  `task-block.sh`, que a su vez se niega igual si se invoca a mano sobre
  una card `Hecha`.
