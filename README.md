...
- Un `/task-start` relanzado por error sobre una card que ya no está libre
  (`Hecha`, `Lista para merge`, `Revisión automática` o `Bloqueada`) lo dice
  en ese mismo turno, sin tocar git ni Notion, en vez de actualizar el
  workspace y preguntar qué hacer (DEVKIT-76: un relanzamiento así sobre una
  card `Hecha` gastó 8 turnos y terminó en `Bloqueada` porque la barrera de
  pregunta abierta no distinguía una card ya cerrada). Esa misma barrera
  (`forzar_task_block` en `devkit-run.sh`) tampoco bloquea una card `Hecha`
  aunque el `result` parezca una pregunta: deja la alarma en `watch.log` y no
  llama a `task-block.sh`, que a su vez se niega igual si se invoca a mano
  sobre una card `Hecha`.
