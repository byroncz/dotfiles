...
| `devkit code <proyecto>` | Abre el editor VS Code del proyecto en el navegador. Úsalo para el trabajo del día a día. |
...
| `ll` | Alias de `ls -lah`. |
| `c` | Alias de `clear`. |
| `devkit-run <skill> <Clave>` | Lanza una skill en segundo plano y se queda mostrando su avance hasta que termina. Úsalo para lanzar cualquier skill a mano, fuera del ciclo automático del bucle. Alias corto: `dk`, mismo script y mismos argumentos. |
| `devkit-run --estado [--seguir]` | Muestra todos los agentes del contenedor y se refresca cada 3 s. Úsalo para saber qué está corriendo; Ctrl-C cierra solo el monitor. |
| `devkit-run --tablero [--seguir]` | Muestra las cards activas del proyecto en una tabla y se refresca cada 30 s. Úsalo para ver de un vistazo qué card está en progreso o bloqueada. |
| `devkit-run --cola` | Muestra las primeras diez cards de la cola, en el orden en que el bucle las va a tomar. Úsalo para ver qué sigue antes de que arranque. |
| `devkit-run --agentes-vivos` | Lista PID, Clave y paso de cada agente en curso en el contenedor, o dice que no hay ninguno. Úsalo antes de un `devkit recreate`/`devkit rebuild` a mano. |
| `devkit-run --pausa` | Pone el interruptor en pausa: el bucle deja de tomar cards nuevas de la cola cuando la obedezca; un lanzamiento manual sigue permitido. |
| `devkit-run --alto` | Pone el interruptor en alto: el bucle se detiene del todo cuando la obedezca y un lanzamiento manual se rechaza. Úsalo antes de una intervención que no debe cruzarse con ningún agente. |
| `devkit-run --reanudar` | Vuelve el interruptor al modo trabajo. |
| `devkit-run pr-review <N>` | Revisa el PR número `<N>` como revisor independiente y publica el veredicto. Úsalo para forzar una revisión sin esperar al bucle. |
| `devkit-run task-fix <N>` | Corrige el PR número `<N>` según los hallazgos de la última revisión. Úsalo para forzar una corrección sin esperar al bucle. |
