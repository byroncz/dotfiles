...
# guarda en Dropbox lo que se borre localmente, por fecha del día del Mac
# (TZ, DEVKIT-64): el corte de día es el suyo, no UTC.
set -u
...
while true; do
  if [ -n "$(find "$local_dir" -newer "$stamp" -not -name '.last-sync' -print -quit 2>/dev/null)" ]; then
    ...
    rclone sync "$local_dir" "$remote" \
      ...
      --backup-dir "${remote%/*}/.trash/$(date +%Y%m%d)" \
      --transfers 2 --checkers 4 --quiet \
      || echo "$(date +%FT%T%:z) sync falló" >&2
  fi
  sleep 60
done
