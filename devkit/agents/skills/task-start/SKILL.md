...
- `task-begin.sh` ya comprobó, antes de que arrancaras, que el workspace
  estaba limpio y que ningún otro agente lo ocupaba
  (`devkit-run --otros-agentes`). Si necesitas repetir esa comprobación más
  adelante en la sesión, tu propio `claude -p` puede aparecer en su
  resultado como `propio: <pid> ...` (DEVKIT-54, DEVKIT-63, DEVKIT-77):
  nunca corras tu propio `ps`/`pgrep` para desconfiar del resultado, ni lo
  interpretes como un agente ajeno.
