...
2. Resuelve `hostname` y `git branch --show-current` como comandos aparte,
   ...
   card con `DEVKIT_HALLAZGO_TITULO` puesta antes del comando: es lo que le
   dice a `task-block.sh` que además del bloqueo cree una card de hallazgo en
   DEVKIT, con `<proyecto>` y `<dominio(s)>` ya resueltos, no un problema de
   la card en curso sino del template (DEVKIT-184: sin esto, cada dominio
   nuevo bloqueado se perdía sin quedar registrado para el template):
