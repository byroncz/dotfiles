...
1. Lee `.claude/devkit-notion.json` para obtener el `data_source_id` de
   Tareas y el código del proyecto. Busca en Proyectos la fila cuyo Código
   coincide; su URL es la relación Proyecto.
...
3. Crea la página con estas propiedades: Título, Estado `Backlog`, Nivel,
   Proyecto, Tipo (`feature`, `bug` o `chore`), Prioridad, Agente `humano`
   si la creó un humano o el agente actual si nace de tu trabajo. Si es hija
   de una Épica, relaciona `Padre` y asigna `Orden`.
