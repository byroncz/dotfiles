...
- Medidas de `devkit rebuild devkit` en el Mac: build de 126,9 s, imagen final
  ...
  --no-stream`), 235 MiB de 7,8 GiB asignados (2,9 %). La imagen previa a
  esta card, con Neovim y tmux todavía dentro (`b33709ccd140`), pesaba
  2,72 GB: la reducción de 180 MB se sostiene en lo que salió del
  `Dockerfile` y del repo, no en una estimación: el paquete `tmux`, el
  binario de Neovim en `/opt/nvim` y unas 830 líneas de configuración y
  pruebas en `devkit/nvim/`.
